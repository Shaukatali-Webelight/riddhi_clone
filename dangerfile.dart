...
void main() {
  ...
  const descriptionShort =
      "This PR's description is too short. It should be contain at least 30 characters.";
  const titleShort =
      "This PR's title is too short. It should be contain at least 15 characters.";
  ...
  const removeBranch =
      'Should have deleted the source branch. Please fill the checkbox.';
...
void checkPubLock(String pubLockContain) {
  final isContainPubYml =
      danger.git.modifiedFiles.any((file) => file == 'pubspec.yml');
  final isContainPubLock =
      danger.git.modifiedFiles.any((file) => file == 'pubspec.lock');
...
void checkDescription(int minimumLength, String descriptionShort) {
  final descriptionIsLongEnough =
      danger.gitLab.mergeRequest.description.length >= minimumLength;
...
void checkAssignees(String noAssignee) {
  final hasAssignees =
      danger.gitLab.mergeRequest.assignee?.name.isNotEmpty ?? false;
...
void checkRemoveBranch(String removeBranch) {
  final isRemoveBranch =
      danger.gitLab.mergeRequest.forceRemoveSourceBranch ?? false;
